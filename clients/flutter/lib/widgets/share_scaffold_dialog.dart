    ...
    final rooms = Matrix.of(context).client.rooms
        .where(
          (room) =>
              ...
              !room.isSpace &&
              // Скрытые из списка чатов комнаты — не адресаты «Поделиться»:
              // обсуждения каналов (topology hidden:true) и сторис
              // (lizaChatType=='stories'). Один предикат покрывает оба класса.
              // Сам канал не режем — репост в свой канал легитимен, а
              // подписчику канал и так не виден (events_default:100 →
              // canSendDefaultMessages=false).
              !room.isHiddenChat &&
              room.membership == Membership.join,
        )
        .toList();
    // Pin Liza assistant DM first in share list — единый канон с главным
    // списком (chat_list.dart): строгий mxid + пустое имя, а не localpart
    // (localpart цеплял мёртвый `@liza:synapse...` DM и mini App-чаты).
    final lizaIdx = rooms.indexWhere(
      (r) => isLizaAssistantRoom(r, MatrixState.lizaMxid),
    );
    if (lizaIdx > 0) {
      rooms.insert(0, rooms.removeAt(lizaIdx));
    }
