  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          // «Скопировать всё» и «Поделиться» берут текст из ФАЙЛА
          // (readLogsText) — это полная история, а экран показывает только
          // outputEvents текущей сессии (живой фильтр/цвета).
          IconButton(
            icon: const Icon(Icons.copy_all_outlined),
            onPressed: () =>
                FileLogger.instance.copyLogsToClipboard(context),
          ),
          IconButton(
            icon: const Icon(Icons.share_outlined),
            onPressed: () => FileLogger.instance.shareLogs(context),
          ),
          IconButton(
