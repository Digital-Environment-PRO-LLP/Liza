...
class LoadingDialogState<T> extends State<LoadingDialog> {
  ...
  /// Свой роут — чтобы завершать ИМЕННО его. `Navigator.pop` снимает верхний
  /// роут навигатора: при двух стековых диалогах первый завершившийся снимал
  /// чужой (и отдавал ему свой результат), а свой висел модально до перезапуска
  /// приложения (инцидент 2026-09-16, Windows).
  ...
  void initState() {
    super.initState();
    widget.future.then(
      (result) => _finish(Result.value(result)),
      onError: (e, s) {
        if (!mounted) return;
        setState(() {
          exception = e;
  ...
  void _finish(Result<T> result) {
    final route = _route;
    if (!mounted || route == null) return;
    final navigator = route.navigator;
    if (navigator == null) return;
    if (route.isCurrent) {
      navigator.pop<Result<T>>(result);
    } else {
