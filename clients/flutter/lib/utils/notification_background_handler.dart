...
void notificationTapBackground(
  ...
  // Адресат — по clientName из payload уведомления (мультиаккаунт), не `.first`.
  final payload = LizaPushPayload.fromString(notificationResponse.payload ?? '');
...
/// [clients] — залогиненные клиенты устройства; адресат тапа выбирается по
/// `payload.clientName` (`clientForPush`). [onClientResolved] — хук для
/// `MatrixState.setActiveClient`: экраны (чат, StoryViewer) читают
/// `Matrix.of(context).client`, поэтому активный аккаунт переключается ДО навигации.
Future<void> notificationTap(
  NotificationResponse notificationResponse, {
  GoRouter? router,
  required List<Client> clients,
  L10n? l10n,
  void Function(Client client)? onClientResolved,
}) async {
  Logs().d(
    'Notification action handler started',
    notificationResponse.notificationResponseType.name,
  );
  final payload = LizaPushPayload.fromString(
  ...
  switch (effectiveNotificationResponseType(notificationResponse)) {
    case NotificationResponseType.selectedNotification:
      ...
      // Сторис-комната → просмотрщик сторис, обычная → чат. Общая навигация с
      // iOS cold-start NSE-тапом (background_push).
      navigatePushTap(
        client: client,
        router: router,
        roomId: roomId,
    ...
    case NotificationResponseType.selectedNotificationAction:
      final actionType = LizaNotificationActions.values.singleWhereOrNull(
      ...
      switch (actionType) {
        case LizaNotificationActions.markAsRead:
        ...
        case LizaNotificationActions.reply:
          ...
          if (PlatformInfos.isAndroid) {
            ...
            final notificationId = pushNotificationId(
              client.clientName,
              room.id,
            );
