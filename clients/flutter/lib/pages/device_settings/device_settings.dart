...
import 'package:liza/pages/device_settings/device_settings_view.dart';
import 'package:liza/pages/key_verification/key_verification_dialog.dart';
...
class DevicesSettingsController extends State<DevicesSettings> {
  ...
  void removeDevicesAction(List<Device> devices) async {
    ...
    // OAuth2 через ProdamusID — у пользователей нет паролей, поэтому UIA-flow
    // (m.login.password) недоступен. На стороне Synapse выставлен
    // ui_auth.session_timeout, благодаря которому свежая сессия пропускается
    // через validate_user_via_ui_auth с can_skip_ui_auth=True. Шлём DELETE
    // без auth — сервер удалит устройство, не показывая UIA-диалог.
    await showFutureLoadingDialog(
      context: context,
      delay: false,
      future: () => matrix.client.deleteDevices(deviceIds),
    );
    reload();
  }
