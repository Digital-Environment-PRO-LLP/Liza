...
/// Диалог ввода названия для кнопки «+» внутри компании.
///
/// Вынесен из [_SpaceViewState], чтобы страж реестра рендерил РЕАЛЬНЫЙ диалог,
/// а не его реплику (тот же приём, что у `showRoomNameInputDialog`): сам
/// [SpaceView] открывает меню под `canChangeStateEvent(SpaceChild)` и зовёт
/// сетевой `singleSpaceService.fetch()` в `initState`.
///
/// Возвращает уже обрезанное непустое название либо `null` (отмена).
/// Текст ошибки тип-НЕЙТРАЛЬНЫЙ намеренно: тип сущности уже назван в `hintText`
/// («Название группы» / «Название пространства»), а терминология
/// «пространство/компания» заморожена постановщиком до концепции компаний.
Future<String?> showAddRoomNameDialog(
  ...
  final name = await showTextInputDialog(
    ...
    // trim и в валидаторе, и в возвращаемом значении: иначе «  Отдел  »
    // проходит гейт и уезжает на сервер вместе с пробелами (LABA-2535).
    validator: (text) =>
...
class _SpaceViewState extends State<SpaceView> {
  ...
  void initState() {
    _loadHierarchy();
    Matrix.of(context).singleSpaceService.fetch().then((_) {
      if (mounted) setState(() {});
    });
    super.initState();
  }
  ...
  /// Слитый список вложенных чатов: sync (room.spaceChildren) первичен,
  /// hierarchy (_discoveredChildren) добавляет метаданные чужих/публичных.
  /// Возвращает SpaceRoomsChunk$2, чтобы itemBuilder не менялся.
  List<SpaceRoomsChunk$2> _mergedSpaceChildren(Room room) {
    final client = room.client;

    final hierarchyById = <String, SpaceRoomsChunk$2>{
      for (final c in _discoveredChildren) c.roomId: c,
    };

    // via пустой у разлинкованных вручную children (сервер их так же
    // отсеивает _has_valid_via) — пропускаем.
    final syncIds = room.spaceChildren
        .where((c) => (c.roomId?.isNotEmpty ?? false) && c.via.isNotEmpty)
        .map((c) => c.roomId!)
        .toSet();

    final chunkById = <String, SpaceRoomsChunk$2>{};
    for (final roomId in syncIds) {
      var joined = client.getRoomById(roomId);
      if (joined?.membership == Membership.leave) joined = null;
      // Сторис-комнаты скрыты в списке чатов и пространствах.
      if (joined != null && joined.isHiddenChat) {
        continue;
      }
      if (joined != null) {
        chunkById[roomId] = SpaceRoomsChunk$2(
          roomId: roomId,
          name: joined.getLocalizedDisplayname(),
          avatarUrl: joined.avatar,
          topic: joined.topic,
          numJoinedMembers: joined.summary.mJoinedMemberCount ?? 0,
          roomType: joined.isSpace ? 'm.space' : null,
          guestCanJoin: false,
          worldReadable: false,
          childrenState: const [],
        );
      } else if (hierarchyById.containsKey(roomId)) {
        chunkById[roomId] = hierarchyById[roomId]!;
      }
      // Иначе: юзер не joined И hierarchy без метаданных. Это приватные чаты,
      // куда юзер не приглашён (Matrix не раскрывает их состояние не-членам).
      // Скрываем: рейл компании показывает только доступное юзеру. Публичные,
      // hierarchy для которых ещё в пути, временно пропадут до подгрузки.
    }

    for (final c in _discoveredChildren) {
      // Пропускаем сторис-комнаты из hierarchy (foreign/публичные).
      final discoveredRoom = client.getRoomById(c.roomId);
      if (discoveredRoom != null && discoveredRoom.isHiddenChat) {
        continue;
      }
      chunkById.putIfAbsent(c.roomId, () => c);
    }

    // Один проход по chunkById: делим на свои (joined, не leave) и чужие.
    final local = <MergeChild>[];
    final foreign = <MergeChild>[];
    for (final e in chunkById.entries) {
      final joined = client.getRoomById(e.key);
      final isLocal = joined != null && joined.membership != Membership.leave;
      (isLocal ? local : foreign).add(
        MergeChild(roomId: e.key, name: e.value.name, isLocal: isLocal),
      );
    }
    final ordered = mergeSpaceChildren(spaceChildren: local, hierarchy: foreign);

    return [for (final m in ordered) chunkById[m.roomId]!];
  }
  ...
  void _onSpaceAction(SpaceActions action) async {
    ...
    switch (action) {
      case SpaceActions.details:
        await space?.postLoad();
        context.push('/rooms/${widget.spaceId}/details/info');
        break;
      ...
      case SpaceActions.members:
        ...
        break;
      case SpaceActions.deleteCompanyViaSupport:
        if (space == null) return;
        await requestCompanyDeletion(context, space);
        return;
      case SpaceActions.leave:
        // LABA-2540: пространство — не чат, и текст про «чат в архиве» здесь
        // был неверен; у чужой компании выход — это отписка.
        final leaveKind = space == null
            ? LeaveActionKind.leaveSpace
            : leaveActionKindFor(context, space);
  ...
  void _addChatOrSubspace(AddRoomType roomType) async {
    final names = await showAddRoomNameDialog(context, roomType);
    ...
    final result = await showFutureLoadingDialog(
      ...
      future: () async {
        ...
        if (roomType == AddRoomType.subspace) {
          // Родитель объявляется в initial_state, иначе single_space_guard
          // режет space как вторую компанию (LABA-2532).
          await createSubspace(parent: activeSpace, name: names);
        } else {
  ...
  void _showSpaceChildEditMenu(BuildContext posContext, String roomId) async {
    ...
    final action = await showMenu<SpaceChildAction>(
      ...
      items: [
        PopupMenuItem(
          ...
          child: Row(
            children: [
              ...
              Flexible(
                child: Text(L10n.of(context).moveToDifferentSpace),
              ),
            ],
          ),
        ),
        ...
        //       const SizedBox(width: 12),
        //       Flexible(child: Text(L10n.of(context).edit)),
        //     ],
        //   ),
        // ),
        PopupMenuItem(
          ...
          child: Row(
            children: [
              ...
              const SizedBox(width: 12),
              Flexible(
                child: Text(L10n.of(context).removeFromSpace),
              ),
            ],
          ),
        ),
  ...
  Widget build(BuildContext context) {
    ...
    final canSeeMembers = room?.canSeeSpaceMembers == true;
    // Та же таблица, что в меню шапки и контекстном меню плитки: до LABA-2533
    // здесь стоял свой `isOwnCompany`-гейт и корзина на leave — SSOT LABA-2540
    // сюда не доходил.
    final leaveKind = room == null
        ? LeaveActionKind.leaveSpace
        : leaveActionKindFor(context, room);
    return Scaffold(
      appBar: AppBar(
        leading: LizaThemes.isColumnMode(context)
            ? null
            : Center(child: CloseButton(onPressed: widget.onBack)),
        automaticallyImplyLeading: false,
        titleSpacing: LizaThemes.isColumnMode(context) ? null : 0,
        title: ListTile(
        ...
        actions: [
          ...
          PopupMenuButton<SpaceActions>(
            ...
            itemBuilder: (context) => [
              ...
              // Приглашать может модератор/админ. Гейт по power level, а НЕ по
              // room.canInvite: в пространствах-компаниях порог invite равен 0,
              // то есть canInvite истинно для любого участника и не отсекает
              // никого — пункт видели все.
              if (canSeeMembers)
                PopupMenuItem(
              ...
              // Список участников пространства — база юзеров компании, поэтому
              // виден только модератору/админу. Тем же правилом закрыт пункт
              // «Пригласить» выше.
              if (canSeeMembers)
                PopupMenuItem(
              ...
              if (leaveKind == LeaveActionKind.deleteCompanyViaSupport)
                PopupMenuItem(
                  value: SpaceActions.deleteCompanyViaSupport,
                  child: Row(
                    mainAxisSize: .min,
                    children: [
      ...
      body: room == null
          ...
          : StreamBuilder(
              ...
              builder: (context, snapshot) {
                ...
                final children = _mergedSpaceChildren(room);
                  ...
                  slivers: [
                    ...
                    SliverList.builder(
                      ...
                      itemBuilder: (context, i) {
                        ...
                        final knownRoom = room.client.getRoomById(
                          item.roomId,
                        );
                        final displayname = hierarchyDisplayname(
                          itemName: item.name,
                          canonicalAlias: item.canonicalAlias,
                          knownRoomDisplayname: knownRoom
                              ?.getLocalizedDisplayname(),
                          emptyChatFallback: L10n.of(context).emptyChat,
                        );
                        if (!displayname.toLowerCase().contains(filter)) {
                          return const SizedBox.shrink();
                        }
                        var joinedRoom = knownRoom;
