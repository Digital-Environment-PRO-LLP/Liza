...
class BootstrapDialogState extends State<BootstrapDialog> {
  ...
  void _createBootstrap(bool wipe) async {
    ...
    // When creating new SSSS keys (wipe), delete the stored recovery key
    // because it belongs to the old SSSS and will be invalid.
    if (wipe) {
      await flutterSecureStorage.delete(key: _secureStorageKey);
      _recoveryKeyTextEditingController.clear();
    }
    ...
    final key = await flutterSecureStorage.read(key: _secureStorageKey);
    if (key == null) return;
  ...
  Widget build(BuildContext context) {
    ...
    var body = const Center(child: CircularProgressIndicator.adaptive());
    ...
    if (bootstrap.newSsssKey?.recoveryKey != null &&
      ...
      switch (bootstrap.state) {
                      ...
                      onPressed: _recoveryKeyInputLoading
                          ...
                          : () async {
                              ...
                              try {
                                ...
                                // If the key was auto-filled from secure
                                // storage, it is stale — delete it so it
                                // won't confuse the user again.
                                flutterSecureStorage
                                    .delete(key: _secureStorageKey)
                                    .catchError((_) {});
                                _recoveryKeyTextEditingController.clear();
                                setState(
        ...
        case BootstrapState.error:
          ...
          body = const Center(child: Icon(Icons.error_outline, color: Colors.red, size: 80));
          buttons.add(
            ElevatedButton(
