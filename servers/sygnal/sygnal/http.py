...
from sygnal._news_audience import should_skip
...
NEWS_AUDIENCE_SKIPPED = Counter(
    "sygnal_news_audience_skipped",
    "Pushes of Liza News posts not sent: device platform is outside the audience",
    labelnames=["platform"],
)
    ...
    async def _handle_dispatch(
        ...
        try:
            ...
            for d in notif.devices:
                ...
                # Пост Liza News не для платформы этого устройства: молча не шлём.
                # НЕ в rejected — иначе Synapse удалит живой pusher.
                skipped_platform = should_skip(notif, d)
                if skipped_platform is not None:
                    NEWS_AUDIENCE_SKIPPED.labels(skipped_platform).inc()
                    log.info(
                        "Skipping Liza News push: platform %s is outside the audience",
                        skipped_platform,
                    )
                    continue
