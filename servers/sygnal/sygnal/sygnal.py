...
class Sygnal:
    def __init__(
        ...
        if sentrycfg["enabled"] is True:
            ...
            from sygnal.sentry_grouping import before_breadcrumb, before_send

            logger.info("Initialising Sentry")
            # ⚠ before_send/before_breadcrumb ОБЯЗАТЕЛЬНЫ, не «улучшение». Голый
            # init(dsn) группировал события по тексту, а текст несёт request_id и
            # notification_id ⇒ 1193 issue при 1193 событиях за 30 дней (прод
            # 2026-09-08), alert-правило «5 за 10 минут на issue» недостижимо
            # НИКОГДА, и семичасовой сбой APNs 2026-08-11 никого не оповестил.
            # Разбор — sentry_grouping.__doc__.
            sentry_sdk.init(
                sentrycfg["dsn"],
