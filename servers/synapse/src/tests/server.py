    ...
    **kwargs: Any,
    ...
    if USE_POSTGRES_FOR_TESTS:
        ...
        if PREPPED_SQLITE_DB_CONN is None:
            ...
            prepped_conn = sqlite3.connect(":memory:")
            # См. synapse/storage/engines/sqlite.py:on_new_connection - это
            # соединение создаётся напрямую через sqlite3.connect, минуя тот
            # хук, поэтому SQLite defensive mode нужно отключить здесь тоже
            # (иначе схема-миграции, пишущие в sqlite_master напрямую,
            # падают с "table sqlite_master may not be modified" на
            # Python 3.12+).
            prepped_conn.setconfig(sqlite3.SQLITE_DBCONFIG_DEFENSIVE, False)
