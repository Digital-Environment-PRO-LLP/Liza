...
from synapse.push.bulk_push_rule_evaluator import (
    BulkPushRuleEvaluator,
    _invite_targets_hidden_room,
)
...
class TestBulkPushRuleEvaluator(HomeserverTestCase):
    ...
    def _federated_invite_push_count(
        self, invitee: str, create_content: JsonDict
    ) -> int:
        """Число push-actions после ВХОДЯЩЕГО федеративного invite invitee.

        Моделируем федеративный invite: событие приходит со stripped state в
        unsigned.invite_room_state (там m.room.create целевой комнаты). Именно
        так guest-сервер узнаёт про комнату при первом контакте (локальной
        state ещё нет). Комнаты в БД нет — эвалюатор опирается только на invite.
        """
        bulk_evaluator = BulkPushRuleEvaluator(self.hs)
    ...
    def test_federated_invite_to_stories_room_suppresses_push(self) -> None:
        """Входящий федеративный invite в скрытую сторис-комнату НЕ генерит
        push (Liza-фикс: единственный путь погасить первый push массового
        backfill, т.к. owner-сервер мьют за чужого юзера не ставит).

        Для обычной комнаты push генерится штатно — это покрыто остальными
        тестами класса (событие проходит мою врезку без раннего return и идёт
        по обычному пути); здесь моделируется только скрытая комната, для
        которой outlier-событие без state в БД корректно гасится ДО обращения
        к power-levels."""
        bob = self.register_user("bob", "pass")
        self.assertEqual(
            self._federated_invite_push_count(
                bob, {"com.liza.stories": True}
            ),
            0,
        )


class InviteTargetsHiddenRoomTestCase(unittest.TestCase):
    """Юнит-тесты для _invite_targets_hidden_room (подавление invite-push в
    скрытые сторис-комнаты, в т.ч. для входящих федеративных invite, где state
    комнаты ещё не персистится и признак берётся из invite_room_state)."""

    def _invite(self, room_state: Any) -> Any:
        return make_event_from_dict(
            {
                ...
                "content": {"membership": "invite"},
                "unsigned": {"invite_room_state": room_state},
            },
            room_version=RoomVersions.V10,
        )

    def _create_state(self, content: JsonDict) -> JsonDict:
        return {
            "type": "m.room.create",
            "state_key": "",
            "sender": "@alice:local.test",
    ...
    def test_stories_room_by_creation_flag(self) -> None:
        event = self._invite([self._create_state({"com.liza.stories": True})])
        self.assertTrue(_invite_targets_hidden_room(event))

    def test_stories_room_by_chat_type(self) -> None:
        event = self._invite(
            [self._create_state({"com.liza.chat.type": "stories"})]
        )
        self.assertTrue(_invite_targets_hidden_room(event))

    def test_normal_room_not_hidden(self) -> None:
        event = self._invite([self._create_state({"name": "Обычный чат"})])
        self.assertFalse(_invite_targets_hidden_room(event))

    def test_no_invite_room_state(self) -> None:
        event = make_event_from_dict(
            {
                "type": "m.room.member",
                "state_key": "@bob:remote.example",
    ...
    def test_malformed_stripped_state_ignored(self) -> None:
        # Не-список / мусорные элементы не должны ронять функцию.
        self.assertFalse(_invite_targets_hidden_room(self._invite("garbage")))
        self.assertFalse(
            _invite_targets_hidden_room(self._invite([None, 42, "x"]))
        )
