...
class LizaReadGraceTests(HomeserverTestCase):
    """Liza-форк: `push.read_grace_ms` — пуш не уходит, если событие прочитано
    (m.read / m.read.private, на любом устройстве) до истечения grace от
    origin_server_ts; не прочитано — уходит с задержкой ≤ grace.
    ledger:RL-push-read-grace
    """
    ...
    def make_homeserver(self, reactor: MemoryReactor, clock: Clock) -> HomeServer:
        ...
        m.post_json_get_json = post_json_get_json
        return self.setup_test_homeserver(proxied_blocklisted_http_client=m)

    def _setup_room_with_pusher(self) -> Tuple[str, str, str, str]:
        user_id = self.register_user("user", "pass")
        access_token = self.login("user", "pass")
        other_user_id = self.register_user("other_user", "pass")
        other_access_token = self.login("other_user", "pass")
        ...
        return user_id, access_token, room_id, other_access_token

    def _event_push_attempts(self, event_id: str) -> list:
        return [
            a
            for a in self.push_attempts
            if a[2]["notification"].get("event_id") == event_id
        ]

    def _send_receipt(
        self, access_token: str, room_id: str, event_id: str, receipt_type: str
    ) -> None:
        channel = self.make_request(
            "POST",
            "/rooms/%s/receipt/%s/%s" % (room_id, receipt_type, event_id),
            {},
            access_token=access_token,
        )
        self.assertEqual(channel.code, 200, channel.json_body)
    ...
    def test_default_off(self) -> None:
        """AC-4: без ключа read_grace_ms поведение прежнее — пуш уходит сразу."""
        self.assertIsNone(self.hs.config.push.push_read_grace_ms)
        _, _, room_id, other_tok = self._setup_room_with_pusher()
        response = self.helper.send(room_id, body="Hi!", tok=other_tok)
        self.pump()
        self.assertEqual(len(self._event_push_attempts(response["event_id"])), 1)

    @override_config({"push": {"read_grace_ms": "3s"}})
    def test_read_private_within_grace_skips_push(self) -> None:
        """AC-1: m.read.private (Liza-клиент шлёт всегда) в окне grace → пуша по
        событию нет; бейдж-апдейт по квитанции — штатный, это не пуш события."""
        _, tok, room_id, other_tok = self._setup_room_with_pusher()
        response = self.helper.send(room_id, body="Hi!", tok=other_tok)
        event_id = response["event_id"]

        self.reactor.advance(1)
        self.assertEqual(self._event_push_attempts(event_id), [])
        self._send_receipt(tok, room_id, event_id, "m.read.private")
        self.reactor.advance(5)
        self.assertEqual(self._event_push_attempts(event_id), [])

    @override_config({"push": {"read_grace_ms": "3s"}})
    def test_read_public_within_grace_skips_push(self) -> None:
        """AC-1: то же для публичного m.read."""
        _, tok, room_id, other_tok = self._setup_room_with_pusher()
        response = self.helper.send(room_id, body="Hi!", tok=other_tok)
        event_id = response["event_id"]
        self.reactor.advance(1)
        self._send_receipt(tok, room_id, event_id, "m.read")
        self.reactor.advance(5)
        self.assertEqual(self._event_push_attempts(event_id), [])

    @override_config({"push": {"read_grace_ms": "3s"}})
    def test_unread_after_grace_sends_push(self) -> None:
        """AC-2/AC-5: не прочитано → пуш уходит после grace, бейдж посчитан
        после сна (1 непрочитанное)."""
        _, _, room_id, other_tok = self._setup_room_with_pusher()
        response = self.helper.send(room_id, body="Hi!", tok=other_tok)
        event_id = response["event_id"]

        self.reactor.advance(2.9)
        self.assertEqual(self._event_push_attempts(event_id), [])
        self.reactor.advance(0.2)
        attempts = self._event_push_attempts(event_id)
        self.assertEqual(len(attempts), 1)
        self.assertEqual(attempts[0][2]["notification"]["counts"]["unread"], 1)
        attempts[0][0].callback({})

    @override_config({"push": {"read_grace_ms": "3s"}})
    def test_receipt_on_older_event_does_not_skip(self) -> None:
        """AC-3: квитанция на БОЛЕЕ РАННЕЕ событие не считается прочтением
        нового — пуш по новому уходит."""
        _, tok, room_id, other_tok = self._setup_room_with_pusher()
        first = self.helper.send(room_id, body="First", tok=other_tok)["event_id"]
        self.reactor.advance(4)
        self.assertEqual(len(self._event_push_attempts(first)), 1)
        self._event_push_attempts(first)[0][0].callback({})
        self.pump()

        second = self.helper.send(room_id, body="Second", tok=other_tok)["event_id"]
        self.reactor.advance(1)
        # Квитанция на первое (старое) событие в окне grace второго.
        self._send_receipt(tok, room_id, first, "m.read.private")
        self.reactor.advance(3)
        self.assertEqual(len(self._event_push_attempts(second)), 1)

    @override_config({"push": {"read_grace_ms": "3s"}})
    def test_batch_does_not_accumulate_delay(self) -> None:
        """AC-2: якорь — origin_server_ts: три события подряд → все три пуша
        не позже grace от СВОЕГО события, задержка не суммируется."""
        _, _, room_id, other_tok = self._setup_room_with_pusher()
        ids = [
            self.helper.send(room_id, body="m%d" % i, tok=other_tok)["event_id"]
            for i in range(3)
        ]
        self.reactor.advance(3.2)
        for event_id in ids:
            attempts = self._event_push_attempts(event_id)
