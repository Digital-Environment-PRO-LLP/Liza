...
class SyncHandler:
    ...
    async def get_sync_result_builder(
        ...
        # chat_topology sync-gate (см. docs/superpowers/specs/2026-07-01-
        # chat-topology-stories-sync-gate-design.md): устройства, не
        # прошедшие version-gate, не должны видеть hidden-комнаты (stories).
        # Нет модульного хука для фильтрации /sync, поэтому модуль
        # прокидывает себя на hs напрямую (см. ChatTopologySyncGateModule).
        chat_topology_module = getattr(self.hs, "liza_chat_topology_sync_gate_module", None)
        if chat_topology_module is not None and chat_topology_module.enabled:
            device_id = sync_config.device_id
            passed_gate = (
                await chat_topology_module.device_passed_gate(device_id)
                if device_id is not None
                else False
            )
            if not passed_gate:
                hidden_room_ids = await chat_topology_module.hidden_rooms.get_hidden_room_ids_for_user(
                    user_id
                )
                mutable_rooms_to_exclude.update(hidden_room_ids)
        ...
        # chat_topology sync-gate: устройство только что прошло gate — после
        # этого первый sync с ним должен принести hidden-комнаты полным
        # стейтом (full_state=True), а не пустой дельтой, иначе клиент не
        # увидит m.room.create/m.room.member и не сможет отрендерить комнату.
        # Маркер одноразовый (consume), выставляется REST-эндпоинтом
        # capabilities при переходе через порог (см. chat_topology.py).
        if chat_topology_module is not None and chat_topology_module.enabled:
            device_id = sync_config.device_id
            if device_id is not None and since_token is not None:
                consumed = await chat_topology_module.forced_full_state.consume(device_id)
                if consumed:
                    newly_visible = await chat_topology_module.hidden_rooms.get_hidden_room_ids_for_user(
                        user_id
                    )
                    forced_newly_joined_room_ids.update(newly_visible)
