    ...
    async def get_local_media_info(
        ...
        while True:
            ...
            if media_info.media_length is not None:
                # Фантом с нулевым телом (`media_length=0`): запись есть, но байт
                # нет. Отдаём честный 404 вместо частичного multipart-EOF —
                # иначе прод-MMR по федерации получал бы «locating media: EOF» →
                # 500 M_UNKNOWN → у клиента вечный спиннер вместо «недоступно»
                # (инцидент 2026-09-01, спека …-web-upload-zero-byte).
                if media_info.media_length == 0:
                    logger.info("Media %s has zero length (phantom)", media_id)
                    respond_404(request)
                    return None
                return media_info
