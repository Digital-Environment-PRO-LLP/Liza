...
def _parse_range_header(
    ...
    """Разбирает заголовок `Range` запроса в пару `(start, end)`, inclusive.

    Возвращает `None`, если Range отсутствует, некорректен, многосоставной,
    неудовлетворим или покрывает весь файл — во всех этих случаях вызывающий
    отдаёт обычный полный ответ 200 (безопасный fallback, поведение без
    изменений). Поддерживается одиночный диапазон: `bytes=START-END`,
    `bytes=START-`, `bytes=-SUFFIX`.
    """
    if file_size is None or file_size <= 0:
        return None

    raw = request.requestHeaders.getRawHeaders(b"range")
    if not raw:
        return None

    value = raw[0].decode("ascii", "replace").strip().lower()
    ...
    if start == 0 and end == file_size - 1:
        # Диапазон покрывает весь файл — 206 не нужен, отдаём обычный 200.
        return None
    return start, end


@implementer(interfaces.IConsumer)
class _ByteRangeConsumer:
    """Оборачивает consumer (`request`) так, что наружу проходят только байты
    диапазона `[start, end]` (inclusive) — для ответа `206 Partial Content`.

    Producer (`ThreadedFileSender`) по-прежнему читает файл с начала; байты
    вне диапазона молча отбрасываются. Так Range-поддержку не нужно встраивать
    в producer, и она работает с любым `Responder`.
    """

    def __init__(self, consumer: IConsumer, start: int, end: int) -> None:
    ...
    def write(self, data: bytes) -> None:
        ...
        self._pos += len(data)
        lo = max(chunk_start, self._start)
        hi = min(self._pos, self._end + 1)
        if lo < hi:
            self._consumer.write(data[lo - chunk_start : hi - chunk_start])
...
async def respond_with_responder(
    ...
    with responder:
        ...
        # Объявляем поддержку Range — видеоплеер Liza по этому заголовку
        # включает потоковый просмотр без полного скачивания файла.
        request.setHeader(b"Accept-Ranges", b"bytes")

        # HEAD: отдаём только заголовки (Accept-Ranges, Content-Length, ...)
        # без тела. Именно на HEAD клиент проверяет поддержку Range перед
        # стримингом видео.
        if request.method == b"HEAD":
            finish_request(request)
            return

        # Range-запрос → 206 Partial Content. Любой невалидный/полный Range
        # `_parse_range_header` вернёт None → отдаём обычный полный 200
        # (поведение без изменений).
        consumer: IConsumer = request
        byte_range = _parse_range_header(request, file_size)
        if byte_range is not None:
            ...
            request.setHeader(
                b"Content-Range",
                b"bytes %d-%d/%d" % (start, end, file_size),
            )
            request.setHeader(b"Content-Length", b"%d" % (end - start + 1,))
            # Перебиваем общий Cache-Control из `add_file_headers` (он ставит
            # `public,max-age=86400` — это для полного 200 на immutable media).
            # 206 с Authorization — приватный ответ; общий кеш/CDN не должен
            # хранить partial-чанки. RFC 9111 §3.5: shared cache по Authorization
            # обязан считать приватным, но не все CDN корректны → ставим явно.
            # `Vary: Range` намеренно НЕ добавляем — корректный кеш и так
            # обязан учитывать Range при сопоставлении ключа (RFC 9111 §4.1),
            # а некорректные кеши на незнакомый Vary-токен лочат запись как
            # no-cache, что нам не нужно для будущей оптимизации полных 200.
            request.setHeader(b"Cache-Control", b"private, no-store")

            # Оптимизация: для локальных файлов (FileResponder) делаем seek
            # вместо чтения с начала и отбрасывания байтов. Без этого seek к
            # середине 500 МБ файла = чтение всех 500 МБ с диска.
            from .media_storage import FileResponder as _FR

            if isinstance(responder, _FR):
                responder.open_file.seek(start)
                # После seek нужно ограничить количество читаемых байт.
                # FileResponder отдаст всё от seek до EOF, но нам нужно
                # только (end - start + 1) байт. Используем _ByteRangeConsumer
                # с offset=0, т.к. seek уже сделан.
                consumer = _ByteRangeConsumer(request, 0, end - start)
            else:
                consumer = _ByteRangeConsumer(request, start, end)

        try:
            await responder.write_to_consumer(consumer)
        except ConsumerRequestedStopError as e:
            logger.debug("Failed to write to consumer: %s %s", type(e), e)
            # Unregister the producer, if it has one, so Twisted doesn't complain
            if request.producer:
