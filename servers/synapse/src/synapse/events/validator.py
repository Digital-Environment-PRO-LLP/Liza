...
class EventValidator:
    def validate_new(self, event: EventBase, config: HomeServerConfig) -> None:
        ...
        if EventContentFields.MENTIONS in event.content:
            # event.content хранится как immutabledict со списками, замороженными
            # в tuple. strict-pydantic 2.12 не принимает ни immutabledict вместо
            # dict, ни tuple вместо list, поэтому размораживаем перед валидацией.
            validate_json_object(
                unfreeze(event.content[EventContentFields.MENTIONS]), Mentions
            )
