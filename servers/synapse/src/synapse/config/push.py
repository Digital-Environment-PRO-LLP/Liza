...
class PushConfig(Config):
    ...
    def read_config(self, config: JsonDict, **kwargs: Any) -> None:
        ...
        # Liza: read-grace — пушер ждёт столько от origin_server_ts события и
        # перед отправкой перепроверяет квитанцию пользователя (m.read /
        # m.read.private); прочитано (на любом устройстве) → пуш не уходит.
        # Default off; включается per-instance (`push.read_grace_ms: 3s`).
        # Гард на truthy (не `is not None`): код общий для всех инстансов на
        # volume, лишний SQL на каждый пуш флота при `0` недопустим.
        self.push_read_grace_ms = None
