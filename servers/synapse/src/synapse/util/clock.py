    ...
    def _looping_call_common(
        ...
        **kwargs: P.kwargs,
        ...
        def wrapped_f(*args: P.args, **kwargs: P.kwargs) -> Deferred:
            ...
            if context.current_context() is not context.SENTINEL_CONTEXT:
                logger.warning(
                    "Expected `looping_call` callback from the reactor to start "
                    "with the sentinel logcontext but saw %s. Resetting to "
                    "sentinel to prevent crash.",
                    context.current_context(),
                )
                context.set_current_context(context.SENTINEL_CONTEXT)
