...
class EventPushActionsWorkerStore(ReceiptsWorkerStore, StreamWorkerStore, SQLBaseStore):
    ...
    def _get_unread_counts_by_room_for_user_txn(
        ...
        if isinstance(self.database_engine, PostgresEngine):
            # GREATEST игнорирует NULL среди аргументов, но при ВСЕХ NULL сам
            # возвращает NULL — а это случай «в комнате не было ни одного
            # ресипта». Тогда ниже `stream_ordering > NULL` и
            # `last_receipt_stream_ordering = NULL` дают UNKNOWN, и комната
            # МОЛЧА выпадает из бейджа. Ветка SQLite такую комнату считает
            # (COALESCE к 0) — расхождение движков при одной декларируемой
            # логике и было дефектом. Прод 2026-09-09 до фикса: 136 комнат у
            # 40 пользователей невидимы агрегату (у одного — 9).
            #
            # ⚠️ Мьют здесь СОЗНАТЕЛЬНО не фильтруется (не «забыли»). После
            # фикса в агрегат войдут и замьюченные комнаты: мьют
            # prospective-only — он не обнуляет уже накопленный notif_count, а
            # ресипт в замьюченную комнату никто не шлёт. Это приемлемо, потому
            # что единственный потребитель числа — push_tools.get_badge_count →
            # counts.unread в payload, а клиент сырому серверному числу НЕ
            # доверяет и считает бейдж сам, исключая мьют
            # (Room.countsTowardAppBadge, инвариант
            # RL-app-badge-native-visible-count). JOIN на push_rules_enable —
            # лишняя цена на пути, который зовётся при КАЖДОМ dispatch пуша.
            # Разбор: RL-badge-counts-receiptless-rooms.
            max_clause = """GREATEST(
