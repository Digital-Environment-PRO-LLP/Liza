...
class ReceiptsWorkerStore(SQLBaseStore):
    ...
    async def get_last_unthreaded_receipt_for_user_in_room(
        self,
        user_id: str,
        room_id: str,
        receipt_types: Collection[str],
    ) -> tuple[str, int] | None:
        """Liza: некэшированная точечная выборка для read-grace пушера
        (`HttpPusher._process_one`). Кэшированные варианты не годятся: их
        инвалидация идёт по replication-стриму, а перепроверка должна видеть
        квитанцию, записанную секунду назад.
        """
        return await self.db_pool.runInteraction(
            "get_last_unthreaded_receipt_for_user_in_room",
