...
class Sqlite3Engine(BaseDatabaseEngine[sqlite3.Connection, sqlite3.Cursor]):
    ...
    def on_new_connection(self, db_conn: "LoggingDatabaseConnection") -> None:
        ...
        # Python 3.12+ включает SQLite defensive mode (SQLITE_DBCONFIG_DEFENSIVE)
        # по умолчанию, что блокирует прямые UPDATE/INSERT/DELETE в
        # sqlite_master. Некоторые схема-миграции (напр.
        # delta/93/04_make_delayed_event_content_text.py) используют PRAGMA
        # writable_schema + прямую правку sqlite_master для смены типа колонки -
        # штатный SQLite-приём, задокументированный самим Synapse (см.
        # комментарий в той миграции: "This strategy is also used by
        # ../50/make_event_content_nullable.py"). Без явного отключения
        # defensive mode эти миграции падают с "table sqlite_master may not
        # be modified" при любом bootstrap на sqlite3 в Python 3.12+.
