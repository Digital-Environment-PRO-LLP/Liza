...
class BaseUploadServlet(RestServlet):
    ...
    async def _get_file_metadata(
        ...
        # Реджект 0-байтной заливки: пустое тело — заведомо невалидное медиа
        # (нечего воспроизвести/скачать). Без гейта клиент, молча приславший 0
        # байт (на Web `readAsBytes()` при OOM вкладки отдаёт пустой буфер без
        # ошибки), создавал бы запись `media_length=0`, а событие рекламировало
        # реальный размер → фантом, не воспроизводимый НИ У КОГО (инцидент
        # 2026-09-01, спека …-web-upload-zero-byte). Fail-loud вместо фантома.
        if content_length == 0:
            raise SynapseError(
                msg="Uploaded media must not be empty",
                code=400,
...
class UploadServlet(BaseUploadServlet):
    ...
    async def on_POST(self, request: SynapseRequest) -> None:
        ...
        response_data: dict = {"content_uri": str(content_uri)}

        # FFprobe-анализ видео: добавляем com.liza.media_profile в response.
        # Клиент использует эти данные для выбора оптимального качества стриминга.
        if media_type and media_type.startswith("video/"):
            try:
                from synapse.media.ffprobe_service import extract_video_metadata

                media_id = str(content_uri).split("/")[-1]
                file_path = self.filepaths.local_media_filepath(media_id)
                metadata = await extract_video_metadata(file_path)
                if metadata:
                    response_data["com.liza.media_profile"] = metadata
            except Exception:
                logger.warning("FFprobe analysis failed for %s", content_uri, exc_info=True)

        respond_with_json(
            request, 200, response_data, send_cors=True
        )
