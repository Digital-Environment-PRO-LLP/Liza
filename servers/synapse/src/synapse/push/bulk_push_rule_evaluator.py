...
def _invite_targets_hidden_room(event: EventBase) -> bool:
    """True, если invite ведёт в скрытую по топологии сторис-комнату Liza.

    Для таких комнат приглашённый НЕ должен получать invite-push: сторис-
    комнаты замьючены на клиенте, а массовый backfill подписок (stories_
    membership) иначе рассылает сотни push. Клиентский/модульный dont_notify
    не спасает для ВХОДЯЩЕГО федеративного invite: на guest-сервере state
    комнаты ещё не персистится (EventContext.for_outlier), а сам owner-сервер
    не может поставить push-rule за чужого пользователя.

    Признак берём из stripped state самого invite (`unsigned.invite_room_state`),
    т.к. это единственный доступный источник для первого федеративного контакта.
    m.room.create входит в дефолтный prejoin-state, поэтому попадает в invite.
    Логика зеркалит legacy-ветку stories_membership._logic.is_hidden_room
    (com.liza.stories / com.liza.chat.type == "stories").
    """
    room_state = event.unsigned.get("invite_room_state")
    if not isinstance(room_state, list):
        return False
    for raw in room_state:
        stripped = parse_stripped_state_event(raw)
        if stripped is None or stripped.type != EventTypes.Create:
            continue
        content = stripped.content
        return (
            content.get("com.liza.stories") is True
            or content.get("com.liza.chat.type") == "stories"
        )
    return False
...
class BulkPushRuleEvaluator:
    ...
    async def _get_rules_for_event(
        ...
        if (
            ...
            if invite_config.get_invite_rule(event.sender) != InviteRule.ALLOW:
                ...
                return {}
            # Liza (LABA-2545): не пушим приглашение от заблокированного.
            # sync уже отбрасывает такие инвайты (handlers/sync.py:2680-2682,
            # `event.sender in ignored_users`), а push спрашивал ДРУГОЙ источник —
            # get_invite_config_for_user (MSC4155/4380), который при выключенном
            # msc4155_enabled всегда отвечает «разрешено». Отсюда был пуш-«призрак»:
            # уведомление о созданном чате приходит, а самого чата в списке нет.
            # ignored_users кэширован (@cached, account_data.py:549) — лишнего
            # обращения к БД нет. Зеркало решения и его страж —
            # synapse_modules.block_guard._logic.should_notify_invite.
            if event.sender in await self.store.ignored_users(invited):
                return {}
            # Liza: invite в скрытую сторис-комнату не уведомляем (иначе
            # массовый backfill подписок шлёт сотни push; см. функцию).
            if _invite_targets_hidden_room(event):
                return {}
    ...
    async def _action_for_event_by_user(
        ...
        # Никому не считаем push (напр. invite в скрытую сторис-комнату или
        # заблокированный InviteRule) → не тратимся на power-levels/профили и
        # не падаем на get_create_event_for_room для ещё не персистнутой
        # (федеративной outlier) комнаты.
