...
# Liza: пуш пропущен, потому что событие прочитано в окне read-grace.
http_push_read_within_grace_counter = Counter(
    "synapse_http_httppusher_read_within_grace_total",
    "Number of push notifications skipped: event read within read-grace",
    labelnames=[SERVER_NAME_LABEL],
)
...
class HttpPusher(Pusher):
    ...
    async def _process_one(self, push_action: HttpPushAction) -> bool:
        ...
        # Liza: read-grace. Ждём от origin_server_ts события (пачка не
        # суммирует задержки) и перепроверяем, не прочитал ли пользователь
        # событие на ЛЮБОМ устройстве. Выборка в _unsafe_process уже отсеяла
        # прочитанное НА МОМЕНТ выборки — этот слой ловит квитанцию, пришедшую
        # за время сна. Только unthreaded-квитанции: HttpPushAction не несёт
        # thread_id; threaded-квитанцию не увидим — пуш уйдёт, как раньше
        # (fail-safe в сторону лишнего пуша, не потерянного).
        if self.push_read_grace_ms:
            grace_diff_ms = (
                read_grace_deadline_ms(
                    event.origin_server_ts, self.push_read_grace_ms
                )
                - self.clock.time_msec()
            )
            if grace_diff_ms > 0:
                await self.clock.sleep(Duration(milliseconds=grace_diff_ms))
            receipt = await self.store.get_last_unthreaded_receipt_for_user_in_room(
                self.user_id,
                event.room_id,
                (ReceiptTypes.READ, ReceiptTypes.READ_PRIVATE),
            )
            if is_read_within_grace(
                receipt[1] if receipt else None, push_action.stream_ordering
            ):
                http_push_read_within_grace_counter.labels(
                    **{SERVER_NAME_LABEL: self.server_name}
                ).inc()
                logger.info(
                    "Skipping push for %s in %s: event %s read within grace (%d ms)",
                    self.user_id,
                    event.room_id,
                    push_action.event_id,
                    self.push_read_grace_ms,
                )
                # True: last_stream_ordering продвигается, backoff сбрасывается —
                # иначе пропуск выглядел бы как сбой доставки и уходил в retry.
                return True
            # Бейдж посчитан до сна — за время grace он мог измениться.
            badge = await push_tools.get_badge_count(
                self.hs.get_datastores().main,
                self.user_id,
                group_by_room=self._group_unread_count_by_room,
